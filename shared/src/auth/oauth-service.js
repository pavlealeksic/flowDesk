...
exports.getProviderById = exports.getAvailableProviders = exports.createOAuthService = exports.OAuthService = exports.OAUTH_PROVIDERS = void 0;
...
class OAuthService {
    constructor(tokenStorage, isElectron = false) {
        this.tokens = new Map();
        this.refreshIntervals = new Map();
        this.tokenStorage = tokenStorage;
        this.isElectron = isElectron;
    }
    static getInstance(tokenStorage, isElectron) {
    ...
    async initialize() {
        try {
            ...
            for (const [providerId, provider] of Object.entries(exports.OAUTH_PROVIDERS)) {
     ...
     */
    async getAuthorizationUrl(providerId, state) {
        const provider = exports.OAUTH_PROVIDERS[providerId];
        if (!provider) {
     ...
     */
    async exchangeCodeForTokens(providerId, code, state) {
        const provider = exports.OAUTH_PROVIDERS[providerId];
        if (!provider) {
            throw new Error(`Unknown OAuth provider: ${providerId}`);
        }
     ...
     */
    async isAuthenticated(providerId) {
        const tokens = await this.getTokens(providerId);
        return tokens !== null;
    }
    /**
    ...
    // Private methods
    async storeTokens(providerId, tokens, provider) {
    ...
    async loadTokensFromStorage(providerId) {
        if (!this.tokenStorage)
            return null;
        const storageKey = `oauth_tokens_${providerId}`;
        const provider = exports.OAUTH_PROVIDERS[providerId];
    ...
    async loadStoredTokens() {
        for (const providerId of Object.keys(exports.OAUTH_PROVIDERS)) {
            try {
    ...
    setupAutoRefresh(providerId, provider) {
    ...
    normalizeUserInfo(providerId, userData) {
...
// Helper functions
const createOAuthService = (tokenStorage, isElectron) => {
    return new OAuthService(tokenStorage, isElectron);
};
exports.createOAuthService = createOAuthService;
const getAvailableProviders = () => {
    return Object.values(exports.OAUTH_PROVIDERS);
};
exports.getAvailableProviders = getAvailableProviders;
const getProviderById = (providerId) => {
    return exports.OAUTH_PROVIDERS[providerId] || null;
};
