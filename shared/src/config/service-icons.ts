...
export const getServiceIcon = (serviceId: string): ServiceIconConfig | null => {
  return SERVICE_ICONS[serviceId] || null;
};

export const getServiceIconUrl = (serviceId: string, preferLocal = false): string => {
  const config = SERVICE_ICONS[serviceId];
  if (!config) return SERVICE_ICONS['custom-template'].localIcon;
...
export const getServiceFallbackUrl = (serviceId: string): string => {
  const config = SERVICE_ICONS[serviceId];
  return config?.fallbackIcon || SERVICE_ICONS['custom-template'].localIcon;
};

export const getServicesByCategory = (): Record<string, ServiceIconConfig[]> => {
  const categories: Record<string, ServiceIconConfig[]> = {};
  
  Object.values(SERVICE_ICONS).forEach(config => {
...
export const getAllServiceIcons = (): ServiceIconConfig[] => {
  return Object.values(SERVICE_ICONS);
};