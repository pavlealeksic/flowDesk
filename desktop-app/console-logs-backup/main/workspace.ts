...
export class WorkspaceManager extends EventEmitter {
  ...
  private cleanupWebContentsView(serviceId: string): void {
    ...
    if (webContentsView) {
      try {
        ...
        if ('destroy' in webContentsView.webContents) {
          (webContentsView.webContents as any).destroy();
        }
      } catch (error) {
        log.warn('Failed to cleanup web contents view:', serviceId, error);
      }
  ...
  private async preloadWorkspaceServices(workspace: Workspace): Promise<void> {
    const MAX_PRELOADED_SERVICES = 5;
    const enabledServices = workspace.services.filter(s => s.isEnabled).slice(0, MAX_PRELOADED_SERVICES);
    ...
    try {
      ...
      for (const service of enabledServices) {
        if (!this.webContentsViews.has(service.id)) {
          ...
          setTimeout(() => this.cleanupInactiveView(service.id), 30 * 60 * 1000); // 30 minutes
          
          // Load URL in background with timeout
          const loadPromise = webContentsView.webContents.loadURL(service.url);
          const timeoutPromise = new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Service load timeout')), 10000)
          );
  ...
  private async cleanupUnusedViews(currentWorkspaceId: string): Promise<void> {
    ...
    for (const [serviceId] of this.webContentsViews) {
