...
export interface FlowDeskAPI {
  ...
  mail: {
    ...
    downloadAttachment(attachment: { filename: string; data: string; mimeType: string }): Promise<APIResponse<{ path: string }>>;
    
    // Event handlers
    onSyncStarted(callback: (data: { accountId: string }) => void): () => void;
    onSyncCompleted(callback: (data: { accountId: string; syncResult: unknown }) => void): () => void;
  ...
  calendar: {
    ...
    onSyncStatusUpdated(callback: (data: { accountId: string; status: unknown }) => void): () => void;
  ...
  // Search API
  searchAPI: {
    search(options: SearchOptions): Promise<APIResponse<{ results: SearchResult[]; total: number }>>;
