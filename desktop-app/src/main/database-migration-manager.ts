...
export class DatabaseMigrationManager {
   ...
   */
  private async ensureSchemaVersionTable(db: SQLite3Database): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      db.exec(createTableSQL, (err) => {
        if (err) {
          ...
          log.debug('Schema version table ensured');
          resolve();
        }
      });
    });
   ...
   */
  private async ensureMigrationsTable(db: SQLite3Database): Promise<void> {
    return new Promise((resolve, reject) => {
      const createTableSQL = `
      ...
      db.exec(createTableSQL, (err) => {
        if (err) {
          ...
          log.debug('Migrations table ensured');
          resolve();
        }
      });
    });
   ...
   */
  private async getCurrentSchemaVersion(db: SQLite3Database): Promise<number> {
    return new Promise((resolve, reject) => {
      db.get(
        'SELECT MAX(version) as version FROM schema_version',
        (err: Error | null, row: SchemaVersionRow | undefined) => {
          if (err) {
   ...
   */
  private async isMigrationApplied(db: SQLite3Database, migrationId: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      db.get(
   ...
   */
  private async recordMigration(db: SQLite3Database, migration: Migration): Promise<void> {
    return new Promise((resolve, reject) => {
      db.run(
        ...
        (err: Error | null) => {
          if (err) {
            ...
            log.info(`Recorded migration: ${migration.id} - ${migration.description}`);
            resolve();
          }
        }
      );
   ...
   */
  private async updateSchemaVersion(db: SQLite3Database, version: number): Promise<void> {
    return new Promise((resolve, reject) => {
      db.run(
   ...
   */
  private async executeMigration(db: SQLite3Database, migration: Migration): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      if (sqlStatements.length === 0) {
        log.debug(`Skipping empty migration: ${migration.id}`);
        resolve();
        return;
      }

      log.debug(`Executing ${sqlStatements.length} SQL statements for migration ${migration.id}`);
      ...
      const executeNext = () => {
        if (statementIndex >= sqlStatements.length) {
          log.info(`Executed migration: ${migration.id} - ${migration.description}`);
          resolve();
          return;
        }
        ...
        db.exec(statement, (err: Error | null) => {
          if (err) {
            log.error(`Failed statement ${statementIndex}/${sqlStatements.length} in migration ${migration.id}:`, statement);
  ...
  async applyAllMigrations(): Promise<boolean> {
    try {
      ...
      if (!existsSync(this.mailDbPath)) {
        log.warn(`Mail database file does not exist: ${this.mailDbPath}`);
        return true;
      }
      
      if (!existsSync(this.calendarDbPath)) {
        log.warn(`Calendar database file does not exist: ${this.calendarDbPath}`);
        return true;
      }
  ...
  private async applyMigrationsToDatabase(dbPath: string, migrations: Migration[]): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      ...
      const db = new sqlite3.Database(dbPath, sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, async (err: Error | null) => {
        if (err) {
          log.error(`Failed to open database ${dbPath}: ${err.message}`);
          resolve(false);
          return;
        }
        ...
        try {
          ...
          db.serialize(async () => {
            try {
              ...
              log.debug(`Current schema version for ${dbPath}: ${currentVersion}`);
              ...
              for (const migration of migrations) {
                ...
                if (!isApplied) {
                  ...
                  log.debug(`Migration already applied: ${migration.id}`);
                }
              }
              ...
              log.info(`Applied ${appliedCount} migration(s) to ${dbPath}`);
              
              db.close((closeErr: Error | null) => {
                if (closeErr) {
                  log.error(`Error closing database: ${closeErr.message}`);
                }
                resolve(true);
              });
            } catch (error) {
   ...
   */
  private async configureDatabaseSettings(db: SQLite3Database): Promise<void> {
    return new Promise((resolve, reject) => {
      db.serialize(() => {
        // Enable foreign key constraints
        db.run('PRAGMA foreign_keys = ON', (err: Error | null) => {
          if (err) log.warn('Could not enable foreign keys:', err.message);
        });
        
        // Set journal mode to WAL for better concurrency
        db.run('PRAGMA journal_mode = WAL', (err: Error | null) => {
          if (err) log.warn('Could not set WAL mode:', err.message);
        });
        
        // Set synchronous mode to NORMAL for balance of safety and performance
        db.run('PRAGMA synchronous = NORMAL', (err: Error | null) => {
          if (err) log.warn('Could not set synchronous mode:', err.message);
        });
        
        // Set temp store to memory
        db.run('PRAGMA temp_store = MEMORY', (err: Error | null) => {
          if (err) log.warn('Could not set temp store:', err.message);
        });
        
        // Set busy timeout to 30 seconds
        db.run('PRAGMA busy_timeout = 30000', (err: Error | null) => {
          if (err) log.warn('Could not set busy timeout:', err.message);
          resolve();
        });
      });
    });
   ...
   * Begin transaction
   */
  private async beginTransaction(db: SQLite3Database): Promise<void> {
    return new Promise((resolve, reject) => {
   ...
   * Commit transaction
   */
  private async commitTransaction(db: SQLite3Database): Promise<void> {
    return new Promise((resolve, reject) => {
   ...
   * Rollback transaction
   */
  private async rollbackTransaction(db: SQLite3Database): Promise<void> {
    return new Promise((resolve, reject) => {
      db.run('ROLLBACK', (err: Error | null) => {
        if (err) {
          log.error(`Failed to rollback transaction: ${err.message}`);
    ...
    return new Promise((resolve) => {
      const db = new sqlite3.Database(dbPath, sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, async (err: Error | null) => {
        ...
        try {
          ...
          const appliedMigrations = await new Promise<MigrationRow[]>((res, rej) => {
            db.all('SELECT * FROM migrations', (err: Error | null, rows: MigrationRow[]) => {
              if (err) {
                if (err.message.includes('no such table')) {
                  res([]);
                } else {
    ...
    try {
      ...
      return new Promise((resolve) => {
        const db = new sqlite3.Database(dbPath, sqlite3.OPEN_READWRITE, (err: Error | null) => {
          ...
          if (!migration.rollback) {
            log.warn(`No rollback SQL provided for migration ${migration.id}`);
            resolve(false);
            return;
          }

          db.exec(migration.rollback, (err: Error | null) => {
            if (err) {
              log.error(`Failed to execute rollback: ${err.message}`);
              db.close();
              resolve(false);
            } else {
              ...
              db.run(
                ...
                (err: Error | null) => {
                  db.close();
                  if (err) {
                    log.error(`Failed to remove migration record: ${err.message}`);
                    resolve(false);
                  } else {
  ...
  async checkDatabaseHealth(dbType: 'mail' | 'calendar'): Promise<{
    ...
    return new Promise((resolve) => {
      const db = new sqlite3.Database(dbPath, sqlite3.OPEN_READONLY, async (err: Error | null) => {
        if (err) {
          resolve({
            healthy: false,
        ...
        try {
          // Check integrity
          const integrityResult = await new Promise<string>((res) => {
          ...
          const fkResult = await new Promise<number>((res) => {
            db.get('PRAGMA foreign_key_check', (err: Error | null, row: any) => {
              res(err ? -1 : (row ? 1 : 0));
            });
          });
          ...
          // Check WAL mode
          const walResult = await new Promise<string>((res) => {
          ...
          for (const table of expectedTables) {
            const tableExists = await new Promise<boolean>((res) => {
              db.get(
          ...
          const migrationCount = await new Promise<number>((res) => {
            db.get('SELECT COUNT(*) as count FROM migrations', (err: Error | null, row: any) => {
  ...
  async repairDatabase(dbType: 'mail' | 'calendar'): Promise<boolean> {
    ...
    try {
      log.info(`Starting database repair for ${dbType} database`);
      
      const sqlite3 = await this.loadSQLite3();
      
      return new Promise((resolve) => {
        const db = new sqlite3.Database(dbPath, sqlite3.OPEN_READWRITE, async (err: Error | null) => {
          if (err) {
            log.error(`Cannot open database for repair: ${err.message}`);
            resolve(false);
            return;
          }
          ...
          try {
            ...
            // Reindex all tables
            await new Promise<void>((res, rej) => {
              db.run('REINDEX', (err: Error | null) => {
                if (err) {
                  log.warn(`REINDEX failed: ${err.message}`);
                  rej(err);
                } else {
                  log.info('Database reindexed successfully');
                  res();
                }
              });
            });
            ...
            await new Promise<void>((res, rej) => {
              db.run('ANALYZE', (err: Error | null) => {
                if (err) {
                  log.warn(`ANALYZE failed: ${err.message}`);
                  rej(err);
                } else {
                  log.info('Database analyzed successfully');
                  res();
                }
              });
            });
